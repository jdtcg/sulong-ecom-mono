  ...
  return (
    <div className="flex flex-col gap-4 mt-4">
      ...
      <div className="flex flex-col gap-2 text-xs">
        ...
        <div className="flex items-center gap-2">
          {product.sizes.map((size) => (
            <div
              className={`cursor-pointer border-1 p-[2px] ${(selectedSize === size
                ? "border-gray-600"
                : "border-gray-300")}`}
              key={size}
      ...
      <button
        ...
        className="bg-gray-800 text-white px-4 py-2 rounded-md shadow-lg flex items-center gap-2 cursor-pointer text-sm font-medium"
      >
        <Plus className="w-4 h-4" />
        Add to Cart
      </button>
